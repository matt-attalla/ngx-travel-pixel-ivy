import { PixelEventName, PixelConfiguration, PixelEventProperties, PixelEventMeta } from './pixel.models';
...
export class PixelService {
   ...
   * @param eventMeta Optional eventId to send to deduplicate events in Facebook Events Manager
   */
  track(eventName: PixelEventName, properties?: PixelEventProperties, eventMeta?: PixelEventMeta): void {
    ...
    if (properties) {
      if(eventMeta) {
        fbq('track', eventName, properties, eventMeta);
      } else { 
        fbq('track', eventName, properties);
      }
    } else {
      if(eventMeta) {
        fbq('track', eventName, {}, eventMeta);
      } else {
        fbq('track', eventName);
      }
    }
   ...
   * @param eventMeta Optional eventId to send to deduplicate events in Facebook Events Manager
   */
  trackCustom(eventName: string, properties?: object, eventMeta?: PixelEventMeta): void {
    if (!isPlatformBrowser(this.platformId)) {
      return;
    }
    ...
    if (properties) {
      if(eventMeta) {
        fbq('trackCustom', eventName, properties, eventMeta);
      } else { 
        fbq('trackCustom', eventName, properties);
      }
    } else {
      if(eventMeta) {
        fbq('trackCustom', eventName, {}, eventMeta);
      } else {
